...
void gpuKernel(double* y_n_real,
	...
	if (print_flag) {
		...
		print_matrix(name, test_real, 13 * 13, 1 * 13 * 13); 
...
// ======================================================================================================================================
	...
	if (threadIdx.x < 13) {            // hard_code: 13 - ch..
		for (int count1 = 0; count1 < 13; ++count1) {     // count1 < 13
			W_reald_shr[blockIdx.x * 13 + threadIdx.x] += out_reald[blockIdx.x * 169 + threadIdx.x * 13 + count1] * s_reald[count1]; 
			W_reald_shr[blockIdx.x * 13 + threadIdx.x] -= out_imagd[blockIdx.x * 169 + threadIdx.x * 13 + count1] * s_imagd[count1]; 

			W_imagd_shr[blockIdx.x * 13 + threadIdx.x] += out_reald[blockIdx.x * 169 + threadIdx.x * 13 + count1] * s_imagd[count1]; 
			W_imagd_shr[blockIdx.x * 13 + threadIdx.x] += out_imagd[blockIdx.x * 169 + threadIdx.x * 13 + count1] * s_reald[count1]; 
		}
	...
	if (threadIdx.x == 0) {			// hard_code: 13 - ch..
		...
		// modifying ====================================
		/*if (threadIdx.x < 13 && blockIdx.x == 0 * 99 * 261) {
			//test_reald[threadIdx.x] = W_reald[blockIdx.x]; 
			//test_imagd[threadIdx.x] = W_reald_shr2[blockIdx.x * 13 + threadIdx.x]; 
		}*/
		// ==============================================
