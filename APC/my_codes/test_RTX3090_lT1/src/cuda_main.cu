...
void gpuKernel(double* y_n_real,
	...
	for (int count1 = 0; count1 < alpha_size; ++count1) {
		...
		// create rho from X
		dim3 dimGrid1 = getDimGrid(batch_size * data_num * X_size); 
		dim3 dimBlock1 = getDimBlock(1); 
		X_to_rho<<< dimGrid1, dimBlock1 >>>(X_reald, 
										  X_imagd, 
										  rho_reald, 
										  rho_imagd, 
										  alpha_reald, 
		...
		// APC algorithm part3
		dim3 dimGrid2 = getDimGrid(batch_size * data_num * X_size); 
		dim3 dimBlock2 = getDimBlock(R_row); 
		complexMatrixInversion<<< dimGrid2, dimBlock2 >>>(output_reald,
														output_imagd,  
														1, 
														13,
	...
	if (print_flag) {
		...
		print_matrix(name, test_real, X_size, 1 * X_size * data_num); 
...
//	if ((blockIdx.x % X_size <= X_size - 2 * N + 2)) {     // blockIdx.x <= X_size - 2 * N + 2
		...
		// modifying ==========================
		for (int thr_row = 0; thr_row < 13; ++thr_row) {
			test_reald[blockIdx.x * 169 + thr_row * 13 + thr_col] = input_reald[blockIdx.x * 169 + thr_row * 13 + thr_col];
			__syncthreads(); 
			test_imagd[blockIdx.x * 169 + thr_row * 13 + thr_col] = input_imagd[blockIdx.x * 169 + thr_row * 13 + thr_col];
			__syncthreads(); 
		}
		// ==================================== 
...
//	if (threadIdx.x < 13) {          // i < 13
		...
		for (int mat_row = 0; mat_row < 13; ++mat_row) {
			if (mat_row == mat_col) {	
				out_reald[blockIdx.x * 169 + mat_row * 13 + mat_col] = 1.0; 
				out_imagd[blockIdx.x * 169 + mat_row * 13 + mat_col] = 0.0; 
			}
			else {
				out_reald[blockIdx.x * 169 + mat_row * 13 + mat_col] = 0.0; 
				out_imagd[blockIdx.x * 169 + mat_row * 13 + mat_col] = 0.0; 
			}
			__syncthreads(); // APPP
		}
...
// Matrix inversion algorithm main body ======================================== 
		...
		__syncthreads();  // AP
			...
			if ((input_reald[blockIdx.x*169+count1*m+count1] == 0) && (input_imagd[blockIdx.x*169+count1*m+count1] == 0)) {
				...
				//if(mat_row == count1) {
				input_reald[blockIdx.x * 169 + count1 * 13 + mat_col] += input_reald[blockIdx.x * 169 + count2 * m + mat_col]; // ch ..
				input_imagd[blockIdx.x * 169 + count1 * 13 + mat_col] += input_imagd[blockIdx.x * 169 + count2 * m + mat_col]; // ch ..

				out_reald[blockIdx.x * 169 + count1 * 13 + mat_col] += out_reald[blockIdx.x * 169 + count2 * m + mat_col]; 
				out_imagd[blockIdx.x * 169 + count1 * 13 + mat_col] += out_imagd[blockIdx.x * 169 + count2 * m + mat_col]; 
				//}
				__syncthreads(); 	
			}
		...
		__syncthreads();  // AP
			...
			for (int mat_row = count1 + 1; mat_row < 13; ++mat_row) {
				...
				input_reald[blockIdx.x * 169 + mat_row * 13 + mat_col] -= (mul_real * input_reald[blockIdx.x * 169 + count1 * m + mat_col]
																		 - mul_imag * input_imagd[blockIdx.x * 169 + count1 * m + mat_col]); 
				input_imagd[blockIdx.x * 169 + mat_row * 13 + mat_col] -= (mul_real * input_imagd[blockIdx.x * 169 + count1 * m + mat_col]
																		 + mul_imag * input_reald[blockIdx.x * 169 + count1 * m + mat_col]);
				__syncthreads(); // APPP

				out_reald[blockIdx.x * 169 + mat_row * 13 + mat_col] -= (mul_real * out_reald[blockIdx.x * 169 + count1 * m + mat_col]
																	   - mul_imag * out_imagd[blockIdx.x * 169 + count1 * m + mat_col]);
				out_imagd[blockIdx.x * 169 + mat_row * 13 + mat_col] -= (mul_real * out_imagd[blockIdx.x * 169 + count1 * m + mat_col]
		...
		for (int mat_row = 0; mat_row < 13; ++mat_row) { 
		out_real_temp = out_reald[blockIdx.x * 169 + mat_row * 13 + mat_col]; 
		out_imag_temp = out_imagd[blockIdx.x * 169 + mat_row * 13 + mat_col]; 
		out_reald[blockIdx.x * 169 + mat_row * 13 + mat_col] = (out_real_temp * input_reald[blockIdx.x * 169 + mat_row * m + mat_row]
		...
		out_imagd[blockIdx.x * 169 + mat_row * 13 + mat_col] = (out_imag_temp * input_reald[blockIdx.x * 169 + mat_row * m + mat_row]
...
//	if (threadIdx.x < 13) {        // i < 13
		W_reald_shr[blockIdx.x * 13 + mat_col] = 0.0; // ch ...................
		W_imagd_shr[blockIdx.x * 13 + mat_col] = 0.0; // ch ...................
...
//	if (threadIdx.x < 13) {            // hard_code: 13 - ch..
		for (int count1 = 0; count1 < 13; ++count1) {     // count1 < 13
			W_reald_shr[blockIdx.x * 13 + mat_col] += out_reald[blockIdx.x * 169 + mat_col * 13 + count1] * s_reald[count1]; 
			W_reald_shr[blockIdx.x * 13 + mat_col] -= out_imagd[blockIdx.x * 169 + mat_col * 13 + count1] * s_imagd[count1]; 

			W_imagd_shr[blockIdx.x * 13 + mat_col] += out_reald[blockIdx.x * 169 + mat_col * 13 + count1] * s_imagd[count1]; 
			W_imagd_shr[blockIdx.x * 13 + mat_col] += out_imagd[blockIdx.x * 169 + mat_col * 13 + count1] * s_reald[count1]; 
...
//	if (threadIdx.x < 13) {
		W_reald_shr2[blockIdx.x*13+mat_col]=W_reald_shr[blockIdx.x*13+mat_col] * rho_reald[blockIdx.x+12]
										  - W_imagd_shr[blockIdx.x*13+mat_col] * rho_imagd[blockIdx.x+12];
		W_imagd_shr2[blockIdx.x*13+mat_col]=W_reald_shr[blockIdx.x*13+mat_col] * rho_imagd[blockIdx.x+12]
										  + W_imagd_shr[blockIdx.x*13+mat_col] * rho_reald[blockIdx.x+12];
//	}
	...
	if (threadIdx.x % 13 == 0) {			// hard_code: 13 - ch..
		...
		for (int count1 = 0; count1 < 13; ++count1) {        // !!! W_imagd <-> W_reald !!!
			W_imagd[blockIdx.x] -= W_reald_shr2[blockIdx.x*13+count1]*y_n_reald[blockIdx.x+12+12*alpha_indx+count1]
								 - W_imagd_shr2[blockIdx.x*13+count1]*y_n_imagd[blockIdx.x+12+12*alpha_indx+count1];		
			W_reald[blockIdx.x] += W_reald_shr2[blockIdx.x*13+count1]*y_n_imagd[blockIdx.x+12+12*alpha_indx+count1]
							     + W_imagd_shr2[blockIdx.x*13+count1]*y_n_reald[blockIdx.x+12+12*alpha_indx+count1];	
		}
