...
void gpuKernel(double* y_n_real,
	...
	for (int count1 = 0; count1 < alpha_size; ++count1) {
		...
		// create rho from X
		dim3 dimGrid = getDimGrid(batch_size * data_num * X_size); 
	...
	if (print_flag) {
		...
		print_matrix(name, test_real, X_size, 0 * X_size); 
...
//	if (threadIdx.x < 13) {            // hard_code: 13 - ch..
		for (int count1 = 0; count1 < 13; ++count1) {     // count1 < 13
			W_reald_shr[thr_batch * data_num * X_size * 13 + blockIdx.x * 13 + mat_col] += out_reald[blockIdx.x * 169 + mat_col * 13 + count1] * s_reald[count1]; 
			W_reald_shr[thr_batch * data_num * X_size * 13 + blockIdx.x * 13 + mat_col] -= out_imagd[blockIdx.x * 169 + mat_col * 13 + count1] * s_imagd[count1]; 

			W_imagd_shr[thr_batch * data_num * X_size * 13 + blockIdx.x * 13 + mat_col] += out_reald[blockIdx.x * 169 + mat_col * 13 + count1] * s_imagd[count1]; 
			W_imagd_shr[thr_batch * data_num * X_size * 13 + blockIdx.x * 13 + mat_col] += out_imagd[thr_batch*data_num*X_size*168+ blockIdx.x * 169 + mat_col * 13 + count1] * s_reald[count1]; 
